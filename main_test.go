...
func prepareChunks(ctx context.Context, t *testing.T, filname string, chunkSize int) (*file.Info, []string) {
	t.Helper()
		...
		for chunkSize := 1; chunkSize < 152; chunkSize += 10 {
			for bufferSize := 1; bufferSize < 152; bufferSize += 10 {
				...
				t.Run(name+"_"+strconv.Itoa(chunkSize)+"_"+strconv.Itoa(bufferSize), func(t *testing.T) {
					...
					fI, chunkPaths := prepareChunks(ctx, t, filename, chunkSize)
...
func TestSimple(t *testing.T) {
	...
	for name, tc := range tcs {
		...
		t.Run(name, func(t *testing.T) {
			ctx := context.Background()
			fI, chunkPaths := prepareChunks(ctx, t, filename, 21)
			got, err := fI.MergeSort(chunkPaths, 1)
			assert.ElementsMatch(t, got, expectedOutput)
			assert.True(t, errors.Is(err, expectedErr))
		})
	}
